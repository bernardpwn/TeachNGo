...
#Student berarti melihat order yang dia pesan
    ...
    if(isset($_POST['userid']) && isset($_POST['userrole'])){
        ...
        $response["data"] = array();
        if ($userrole == 'Teacher'){
            $q=mysqli_query($conn,"SELECT Order_Id, Orders.Class_Id, Orders.User_Id AS Student_Id, Order_Status FROM Orders JOIN Class ON Orders.Class_Id = Class.Class_Id JOIN User ON Orders.User_Id = User.User_Id WHERE Class.User_Id = $userid AND Order_Status = 'Completed'");
            ...
            $q=mysqli_query($conn,"SELECT Order_Id, Orders.Class_Id, Class.User_Id AS Teacher_Id, Order_Status FROM Orders JOIN Class ON Orders.Class_Id = Class.Class_Id JOIN User ON Orders.User_Id = User.User_Id WHERE Orders.User_Id = $userid AND Order_Status = 'Completed'");
            while($r = $q -> fetch_assoc()){
                $mhs = array();
                $mhs["Order_Id"] = $r["Order_Id"];
                $mhs["Class_Id"] = $r["Class_Id"];
